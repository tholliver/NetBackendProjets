...
public static class IdentityApiCustomerEndpoint
{
    ...
    public static IEndpointConventionBuilder MapIdentityCustomerApi(this IEndpointRouteBuilder endpoints)
    {
        ...
        (UserManager<Customer> userManager, CreateCustomerDto customer) =>
        {
            Console.WriteLine("Here the new");
        ...
        routeGroup.MapPost("/login", async Task<Results<Ok<AccessTokenResponse>, EmptyHttpResult, ProblemHttpResult>>
            (LoginDto login, [FromQuery] bool? useCookies, [FromQuery] bool? useSessionCookies, [FromServices] IServiceProvider sp) =>
        {
        ...
        routeGroup.MapGet("/info", async Task<Results<Ok<CustomerInfoResponse>, ValidationProblem, NotFound>>
       ...
       where TUser : class
    {
        ...
        Console.WriteLine($"Here on {user}");
        return new()
        {
            Email = await userManager.GetEmailAsync(user) ?? throw new NotSupportedException("Users must have an email."),
            IsEmailConfirmed = await userManager.IsEmailConfirmedAsync(user),
